...
class ParentTable extends Component {
    ...
    return (
      <>
        <FormPopover
          ...
          isSaving={loading.effects['ledgerAccount/saveParent']}
        >
          <Button type="link">
  ...
  renderDelBtn = row => {
    ...
    if (loading.effects['ledgerAccount/delPRow'] && delRowId === row.id) {
    ...
    return {
      ...
      onSelectChange: (_, [selectedItem]) => {
        dispatch({
          ...
          dispatch({
            type: 'ledgerAccount/getConfigById',
            payload: {
              id: selectedItem.id,
            },
          });
