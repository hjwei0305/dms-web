...
class ConfigModelFields extends Component {
  ...
  handleAddAuditFields = () => {
    ...
    dispatch({
      type: 'masterDataRegister/addAuditFields',
      payload: {
  ...
  save = data => {
    const { dispatch } = this.props;
    dispatch({
      type: 'masterDataRegister/saveModelField',
      payload: {
        ...data,
      },
    }).then(res => {
  ...
  del = record => {
    ...
    this.setState(
      ...
      () => {
        dispatch({
          type: 'masterDataRegister/deleteModelFields',
          payload: [record.id],
        }).then(res => {
          if (res.success) {
            this.setState({
  ...
  renderDelBtn = row => {
    ...
    if (loading.effects['masterDataRegister/deleteModelFields'] && delRowId === row.id) {
  ...
  getFormModalProps = () => {
    const { loading, masterDataRegister, dataModel } = this.props;
    const { fieldModalVisible, rowData } = masterDataRegister;
    ...
    return {
      ...
      saving: loading.effects['masterDataRegister/saveModelField'],
    };
  };
  ...
  render() {
    const { masterDataRegister, dataModel } = this.props;
    const { fieldModalVisible } = masterDataRegister;
